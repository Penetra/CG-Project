...
int Wall::intersection(Ray &ray, double &t) {
	...
	if(limited){
		if(hit.x > centre.x + squareSize*numberOfSquares || hit.x < centre.x || hit.z > centre.z + squareSize*numberOfSquares || hit.z < centre.z)
			return 0;
	}

	if(chess){
		int x,z;
		x = int(floor((centre.x + hit.x) * 1/squareSize));
		z = int(floor((centre.z + hit.z) * 1/squareSize));
