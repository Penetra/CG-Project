...
Cylinder::Cylinder(double bottomX, double bottomY, double bottomZ, double red, double green, double blue, double r, double h) {
    ...
    top_ = new Wall(0, top.y, 0, 0.0, 1.0, 0.0, 1.0, 1.0, 1.0);
}
...
int Cylinder::intersection(Ray &ray, double &t) {
    ...
    if (intersection.y <= top.y && intersection.y >= bottom.y) {
        if (intersection2.y <= top.y && intersection2.y >= bottom.y) {
            if (t1 < t2 && t1>-0.00000001 && t2 >-0.00000001)
                t = t1;
            else {
                if (t2>-0.00000001)
                    t = t2;
...
void Cylinder::calculateNormal(Point &hitPoint, Vector &normal) {
    /*printf("%lf\n",hitPoint.y);*/
    //if (hitPoint.y <= top.y + 0.000000001 && hitPoint.y >= top.y - 0.000000001 ) { /* Hit the top */
    if (abs(hitPoint.y -top.y) < 0.000000001){
        normal.x = 0;
        normal.y = 1;
        normal.z = 0;
    }else if(abs(hitPoint.y -bottom.y) < 0.000000001){
        //else if (hitPoint.y <= bottom.y + 0.000000001 && hitPoint.y >= bottom.y - 0.000000001) { /* Hit the bottom */
        normal.x = 0;
        normal.y = -1;
        normal.z = 0;
    } else { /* Hit the side */
